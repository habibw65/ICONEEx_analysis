...
def plot_enhanced_stacked_area_chart(df):
    ...
    stack_handles = ax.stackplot(counts.index, counts['RS only'], counts['RS + In-situ'], 
                                 labels=['RS only', 'RS + In-situ'], 
                                 colors=['#0072B2', '#D55E00'], alpha=0.8)
    
    # Add total publications line
    total_publications = counts.sum(axis=1)
    line_handle = ax.plot(total_publications.index, total_publications, color='black', linestyle='--', 
                          label='Total Publications', linewidth=1.5)
    ...
    plt.tight_layout()
    plt.savefig('figure_rs_trends_stacked_area_enhanced.png', dpi=300)

if __name__ == '__main__':
    ris_file = 'savedrecs.ris'
    papers_df = parse_ris(ris_file)
    categorized_df = categorize_papers(papers_df)
    plot_enhanced_stacked_area_chart(categorized_df)
