...
if __name__ == '__main__':
    ...
    print("--- Finding Optimal VI-Correlation Combinations for Each Season (2024) ---")
    seasonal_best_combos = {}

    for season_name, week_range in seasons.items():
        seasonal_best_combos[season_name] = {}
        seasonal_data = merged_data[merged_data['Week'].isin(week_range)]
        
        if seasonal_data.empty:
            print(f"  No data for {season_name} season. Skipping.")
            continue
        ...
        for lrc_param in lrc_parameters:
            ...
            if best_combo_details:
                seasonal_best_combos[season_name][lrc_param] = best_combo_details
                print(f"    Best combo for '{lrc_param}': {best_combo_details['Function']} with {best_combo_details['VI']} (R²: {best_combo_details['R2']:.3f})")
            else:
                print(f"    No suitable combo found for '{lrc_param}' in {season_name}.")
