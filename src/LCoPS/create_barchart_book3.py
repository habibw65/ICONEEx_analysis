...
excel_file_path = '/Users/habibw/Documents/LCoPS/Book3.xlsx'
output_dir = '/Users/habibw/Documents/LCoPS'
output_plot_path = os.path.join(output_dir, 'landcover_barchart_book3.png')
...
try:
    ...
    area_col = "Total area (ha)"

    # Convert land_cover_col to string type at the beginning
    df[land_cover_col] = df[land_cover_col].astype(str)

    # Exclude the row where land_cover_col contains 'Total'
    df = df[df[land_cover_col].str.contains('Total') == False]

    # Remove rows where land_cover_col is 'nan'
    df = df[df[land_cover_col] != 'nan']
    ...
    category_color_map = {
        ...
        'Scrub': '#e3d580',
        'Hedgerows': '#712b6b',
        'Dry Grassland': '#f7ea48', # Assign specific color for Dry Grassland
        'Bracken': '#8B4513', # Assign specific color for Bracken
    ...
    land_cover_to_category = {
        ...
        'Woodland': 'Forests',
        'Hedgerows': 'Hedgerows',
        'Dry Grassland': 'Dry Grassland', # Map Dry Grassland to itself for distinct color
        'Broadleaved Forest and Woodland': 'Forests',
        'Ways': 'Others',
        'Bracken': 'Bracken', # Map Bracken to itself for distinct color
    ...
    df['category'] = df[land_cover_col].apply(lambda x: land_cover_to_category.get(x, 'Others'))
