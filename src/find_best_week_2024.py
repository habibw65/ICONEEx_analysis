...
def load_lrc_parameters(gurteen_dir):
    ...
    if os.path.exists(lrc_2024_path):
        return pd.read_csv(lrc_2024_path)
    else:
        return pd.DataFrame()

# --- Correlation & Modeling Functions (re-used) ---
...
def model_and_plot_nee_for_week(best_week_details, tower_base_dirs, satellite_data_weekly, gurteen_dir):
    ...
    print(f"\n--- Modeling NEE for Best Week of 2024: {target_week} ---")
    ...
    for tower_name, tower_dir in tower_base_dirs.items():
        ...
        tower_satellite_data_week = satellite_data_weekly[
            ...
            (satellite_data_weekly['Year'] == 2024) &
        ...
        try:
            ...
            if np.sum(valid_indices) > 0:
                all_towers_comparison_data.append({
                    'Tower_Year': f'{tower_name} - 2024',
                    'Measured NEE': np.mean(y_observed[valid_indices]),
                    'Modeled NEE': np.mean(y_predicted[valid_indices])
                })
        except Exception as e:
            print(f"Could not model NEE for {tower_name} 2024 Week {target_week}: {e}")
    ...
    ax.set_title(f'Weekly Mean NEE Comparison (Best Week of 2024: {target_week})')
    ax.set_xlabel('Tower')
    ...
    plt.savefig(os.path.join(combined_plots_dir, f'best_week_2024_nee_comparison_Week{target_week}.png'))
    plt.close()
    print(f"Best week 2024 NEE comparison bar chart saved to {combined_plots_dir}")
...
if __name__ == '__main__':
            ...
            for vi in vegetation_indices:
                for func_name, func in correlation_functions.items():
                    ...
                    try:
                        ...
                        result_entry = {'Week': week, 'LRC_Parameter': lrc_param, 'VI': vi, 'Correlation_Type': func_name, 'R2': r2}
                        all_weekly_results.append(result_entry)
                        if r2 > best_r2:
                            best_r2 = r2
    ...
    if not weekly_scores:
        print("Could not find a suitable week in 2024 with valid correlations for all LRC parameters.")
    else:
        scores_df = pd.DataFrame(weekly_scores)
        best_week_details = scores_df.loc[scores_df['Avg_R2'].idxmax()]
        best_week_num = best_week_details['Week']
        
        print("--- Optimal Week Found for 2024 ---")
        print(f"Best Week for Modeling: {best_week_num} (Avg R²: {best_week_details['Avg_R2']:.3f})")
        ...
        print(f"\n--- Detailed Correlation Results for Week {best_week_num} (2024) ---")
        all_results_df = pd.DataFrame(all_weekly_results)
        detailed_results_for_best_week = all_results_df[all_results_df['Week'] == best_week_num]
        print(detailed_results_for_best_week.pivot_table(index=['LRC_Parameter', 'VI'], columns='Correlation_Type', values='R2').round(3))

        model_and_plot_nee_for_week(best_week_details, base_directories, satellite_data_2024, gurteen_dir)
