...
if __name__ == '__main__':
    ...
    # --- 1. Aggregate Data by Weekly Mean ---
    # Select only numeric columns for mean aggregation
    numeric_cols = lrc_parameters + vegetation_indices + ['MODIS_PPFD']
    weekly_mean_data = merged_data.groupby('Week')[numeric_cols].mean().reset_index()

    # --- 2. Find Best Predictors from Mean Data ---
    print("--- Finding Best Predictors from Weekly Mean Data (2024) ---")
    global_best_models = {}
    for lrc_param in lrc_parameters:
        ...
        for vi in vegetation_indices:
            for func_name, func in correlation_functions.items():
                subset_data = weekly_mean_data.dropna(subset=[lrc_param, vi])
    ...
    # --- 3. Apply Global Model and Evaluate Weekly ---
    print("\n--- Applying Global Model (from Mean Data) to Predict Weekly NEE ---")
    weekly_results = []
    ...
    fig.suptitle('Global Model (from Weekly Mean VIs): Measured vs. Modeled Weekly NEE (2024)', fontsize=16, y=0.95)
    ...
    plt.savefig(os.path.join(combined_plots_dir, 'global_model_mean_vi_weekly_nee_comparison.png'))
    plt.close()
    print(f"Global model (from mean VIs) comparison plot saved to {combined_plots_dir}")
