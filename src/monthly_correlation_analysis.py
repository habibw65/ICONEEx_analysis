...
if __name__ == '__main__':
    ...
    # Add Month column (assuming Week 1-4 is Jan, 5-8 is Feb, etc. - simplified for monthly aggregation)
    # A more robust way would be to merge with a date lookup table or use original DateTime if available
    # For now, we'll use a simple mapping based on week number
    merged_data['Month'] = ((merged_data['Week'] - 1) // 4) + 1
    merged_data['Month'] = merged_data['Month'].clip(1, 12) # Ensure month is within 1-12
    ...
    # Aggregate data by Month, Tower, and Year
    numeric_cols_for_monthly_agg = lrc_parameters + vegetation_indices
    monthly_aggregated_data = merged_data.groupby(['Tower', 'Year', 'Month'])[numeric_cols_for_monthly_agg].mean().reset_index()

    best_monthly_combos = {}

    print("--- Finding Optimal VI-Correlation Combinations for Monthly Data (2024) ---")
    for lrc_param in lrc_parameters:
        ...
        for vi in vegetation_indices:
            for func_name, func in correlation_functions.items():
                # Use the monthly_aggregated_data for fitting
                subset_data = monthly_aggregated_data.dropna(subset=[lrc_param, vi])
