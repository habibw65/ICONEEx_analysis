...
if __name__ == '__main__':
    ...
    # Define seasons by week ranges
    seasons = {
        'Winter': list(range(1, 9)) + list(range(48, 53)), # Jan-Feb, Dec
        'Spring': list(range(9, 22)), # Mar-May
        'Summer': list(range(22, 35)), # Jun-Aug
        'Autumn': list(range(35, 48)) # Sep-Nov
    }
    ...
    print("--- Analyzing Seasonal Correlations (2024) ---")
    seasonal_correlation_results = []

    for season_name, week_range in seasons.items():
        seasonal_data = merged_data[merged_data['Week'].isin(week_range)]
        
        if seasonal_data.empty: continue

        for lrc_param in lrc_parameters:
            for vi in vegetation_indices:
                best_r2_for_season_param_vi = -1
                best_model_for_season_param_vi = None

                for func_name, func in correlation_functions.items():
                    subset_data = seasonal_data.dropna(subset=[lrc_param, vi])
                    ...
                    try:
                        ...
                        if r2 > best_r2_for_season_param_vi:
                            best_r2_for_season_param_vi = r2
                            best_model_for_season_param_vi = {
                                'Season': season_name,
                                'LRC_Parameter': lrc_param,
                                'VI': vi,
                                'Correlation_Type': func_name,
                                'R2': r2,
                                'Params': params.tolist() # Convert numpy array to list for easier storage
                            }
                    except (RuntimeError, ValueError): continue
                
                if best_model_for_season_param_vi:
                    seasonal_correlation_results.append(best_model_for_season_param_vi)

    seasonal_results_df = pd.DataFrame(seasonal_correlation_results)
    print("\nSeasonal Correlation Results:")
    print(seasonal_results_df.pivot_table(index=['Season', 'LRC_Parameter'], columns='VI', values='R2').round(3))

    # --- Identify Best Overall Seasonal Models ---
    print("\n--- Best Overall Seasonal Models (Highest R² per LRC Parameter per Season) ---")
    final_seasonal_models = {}
    for season_name in seasons.keys():
        final_seasonal_models[season_name] = {}
        for lrc_param in lrc_parameters:
            season_param_df = seasonal_results_df[
                (seasonal_results_df['Season'] == season_name) &
                (seasonal_results_df['LRC_Parameter'] == lrc_param)
            ]
            if not season_param_df.empty:
                best_fit = season_param_df.loc[season_param_df['R2'].idxmax()]
                final_seasonal_models[season_name][lrc_param] = best_fit
                print(f"  {season_name} - LRC '{lrc_param}': {best_fit['Correlation_Type']} with {best_fit['VI']} (R²: {best_fit['R2']:.3f})")
            else:
                print(f"  {season_name} - LRC '{lrc_param}': No successful model found.")

    # --- Apply Seasonal Models and Evaluate Weekly ---
    print("\n--- Applying Seasonal Models to Predict Weekly NEE ---")
    ...
    for tower_name, tower_dir in base_directories.items():
        ...
        for week in range(1, 53):
            ...
            current_season = None
            for s_name, w_range in seasons.items():
                if week in w_range:
                    current_season = s_name
                    break
            
            if not current_season or not final_seasonal_models[current_season]: continue

            try:
                a_model = final_seasonal_models[current_season].get('a')
                b_model = final_seasonal_models[current_season].get('b')
                c_model = final_seasonal_models[current_season].get('c')
                ...
                if np.sum(valid_indices) > 0:
                    weekly_seasonal_results.append({
                        'Tower': tower_name, 'Week': week, 'Season': current_season,
                        'Measured_NEE': np.mean(y_observed[valid_indices]),
                        'Modeled_NEE': np.mean(y_predicted[valid_indices])
                    })
            except Exception: continue

    seasonal_results_df_plot = pd.DataFrame(weekly_seasonal_results)

    if seasonal_results_df_plot.empty:
        print("No data to plot for seasonal models.")
        exit()

    # --- 4. Visualize and Report ---
    print("\n--- Generating Time Series Comparison Plots for Seasonal Models ---")
    ...
    fig.suptitle('Seasonal Models: Measured vs. Modeled Weekly NEE (2024)', fontsize=16, y=0.95)

    for i, (tower_name, ax) in enumerate(zip(base_directories.keys(), axes.flatten())):
        tower_data = seasonal_results_df_plot[seasonal_results_df_plot['Tower'] == tower_name]
        if tower_data.empty: 
    ...
    plt.savefig(os.path.join(combined_plots_dir, 'seasonal_model_weekly_nee_comparison.png'))
    plt.close()
    print(f"Seasonal model comparison plot saved to {combined_plots_dir}")
